...
def run_exp(cfg: DictConfig):
    ...
    if use_peft:
        assert (lora_r is not None) ^ (lora_relative_r is not None), "Please specify lora_r or lora_relative_r"
    ...
    # 🚀 关键修改：从配置中获取保存路径
    # 使用 cfg.get("output_dir", ...) 来获取命令行传入的路径，如果未传入则使用默认值
    save_dir = cfg.get("output_dir", os.path.join("results", "ZN_mistral"))
