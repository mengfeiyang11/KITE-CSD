...
from logTrainer import LogTrainer, FlatLoraTrainer
import logging
import wandb
...
def causalLMEncode(example, tokenizer, max_length=-1, ignore_masked_token=True):
    # ===== 字段映射，支持不同数据集格式 =====
    if "x" in example and "y" in example:
        inp = example["x"]
        out = example["y"]
    elif "input_text" in example and "target_text" in example:
        inp = example["input_text"]
        out = example["target_text"]
    else:
        raise KeyError("❌ Example dict must contain either (x,y) or (input_text,target_text).")

    is_list_input = isinstance(inp, list)

    # ===== 拼接输入和输出，并加上 EOS =====
    combined_text = (
        [x + " " + y + tokenizer.eos_token for (x, y) in zip(inp, out)]
        if is_list_input
        else inp + " " + out + tokenizer.eos_token
    )
    ...
    input_text_length = (
        [
            len(tokenizer(inp[i], return_tensors="pt")["input_ids"][0])
            for i in range(len(inp))
        ]
        if is_list_input
        else len(tokenizer(inp, return_tensors="pt")["input_ids"][0])
    )

    if (is_list_input and input_text_length[0] >= max_length) or (
        not is_list_input and input_text_length >= max_length
    ):
        print(
            f"⚠️ Input text length >= max_length: {input_text_length} >= {max_length}. "
            "Consider increasing max_length to avoid truncation."
        )

    # ===== 创建 labels，把输入部分 mask 掉 =====
...
def initialize_text_to_text_model(
    ...
    gradient_checkpointing: bool = True  # 添加这个参数
):
...
def train_text_to_text_model(
    run_name: str, # be careful, for qwen expriments, run_name should includes "qwen" to not use compute_metrics
    ...
    **kwargs,
    ...
    TrainerClass = TrainerClass if rho == 0 else FlatLoraTrainer
    if use_loraplus:
        additional_kwargs = {
    ...
    trainer = TrainerClass(
        ...
        compute_metrics=compute_metrics if "qwen" not in run_name else None,
        callbacks=[
            EarlyStoppingCallback(
